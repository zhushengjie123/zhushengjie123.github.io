...
~~~js
 computed: {//person表示该模块化名字，后面的对象是state中的属性
    ...mapState("person", { personList: "personList", poem: "poem" }),
  },
 mounted(){
     this.$store.dispatch('person/poem')//person模块下的action方法poem
     //...mapActions("person", { getPoem: "poem" }),通过mapAction映射来传参修改vuex数据，
     //此处将this.getPoem映射成了 this.$store.dispatch('person/poem')	
     this.$store.commit("person/ADDPERSON", personObj);//当不涉及复杂逻辑比如异步调用api时可以直接跳过dispath而使用commit
}
